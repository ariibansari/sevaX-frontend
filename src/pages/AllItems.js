                ...
                {items.map((item, i) => {
                  return (
                    <div className='item' key={i} onClick={()=>navigate(`/item/${item.item_id}`)}>
                      ...
                      <div className='requests-container'><FaUserFriends /><div className='requests'><span className={`${item.request_count>0 && 'text-success'}`}>{item.request_count}</span> requests</div></div>
